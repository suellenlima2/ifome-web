...
export default function PerfilPage() {
  ...
  const [restrictions, setRestrictions] = useState<RestrictionKey[]>(profile?.restrictions ?? []);
  const [showAllHistory, setShowAllHistory] = useState(false);
  const photoInputRef = useRef<HTMLInputElement>(null);

  const { register, handleSubmit, formState: { errors } } = useForm<ProfileForm>({
    resolver: zodResolver(profileSchema),
    values: profile ? { name: profile.name, email: profile.email, phone: profile.phone, campus: profile.campus } : undefined,
  });
  ...
  if (isLoading) return (
    <div className="col gap-20" style={{ maxWidth: 920, margin: '0 auto' }}>
      <Skeleton h={200} r={14} />
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="col gap-20" style={{ maxWidth: 920, margin: '0 auto', width: '100%' }}>
        <div className="row gap-12">
          <Link href="/student/home"><Button variant="ghost" size="sm" icon={ArrowLeft}>Início</Button></Link>
        </div>

        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ height: 100, background: 'linear-gradient(135deg, var(--green-700) 0%, var(--green-500) 100%)' }} />
          <div className="row gap-16" style={{ padding: '0 24px 20px', marginTop: -40, alignItems: 'flex-end' }}>
            <Avatar name={profile.name} size="xl" />
            <div className="col" style={{ flex: 1, paddingBottom: 6 }}>
              <span style={{ fontSize: 22, fontWeight: 700 }}>{profile.name}</span>
              <span className="text-sm muted">{profile.curso} · Matrícula {profile.matricula}</span>
            </div>
            <Button type="button" variant="secondary" size="sm" icon={Pencil} onClick={() => photoInputRef.current?.click()}>Editar foto</Button>
          </div>
        </div>
        ...
        <div className="row gap-12" style={{ justifyContent: 'space-between' }}>
          <Button type="button" variant="ghost" size="sm" icon={LogOut} style={{ color: 'var(--error)' }} onClick={() => doLogout()}>Sair da conta</Button>
          <div className="row gap-12">
            <Link href="/student/home"><Button type="button" variant="secondary">Cancelar</Button></Link>
